...
IUSE="doc examples +gmpy gtk +imaging +ipython latex mathml opengl pdf png +pyglet +system-mpmath test texmacs theano"
...
RDEPEND="
	...
	system-mpmath? ( >=dev-python/mpmath-0.19[${PYTHON_USEDEP}] )
...
python_prepare_all() {
	if use system-mpmath; then
		...
		epatch "${FILESDIR}"/${P}-system-mpmath.patch
	fi
