...
src_install() {
	...
	pushd "${ED%/}${INSDESTTREE}"/lib_pypy > /dev/null || die
	"${PYTHON}" _audioop_build.py || die "Failed to build audioop (cffi)"
	"${PYTHON}" _curses_build.py || die "Failed to build curses (cffi)"
	"${PYTHON}" _syslog_build.py || die "Failed to build syslog (cffi)"
	"${PYTHON}" _pwdgrp_build.py || die "Failed to build pwdgrp (cffi)"
	if use sqlite; then
		"${PYTHON}" _sqlite3_build.py || die "Failed to build sqlite3 (cffi)"
	fi
	if use tk; then
		"${PYTHON}" _tkinter/tklib_build.py || die "Failed to build tkinter (cffi)"
	fi
	if use gdbm; then
		"${PYTHON}" _gdbm_build.py || die "Failed to build gdbm (cffi)"
	fi
	popd > /dev/null || die

	# Cleanup the building scripts
	find "${ED%/}${INSDESTTREE}" -name "_*build.py" -delete || die

	# Cleanup temporary objects
	find "${ED%/}${INSDESTTREE}" -name "_*cffi.[co]" -delete || die
